...
4. Decide if this is a **read** (a question about the grant
   tracker) or a **write** (asks you to add a row, update a status,
   change an owner, or edit a cell). When ambiguous, treat it as a
   read and ask one clarifying question.
5. For reads: pick the smallest set of `google-sheets` queries that
   answer the question. Format the result per the SOUL "Read-only
   questions" guidance — short, funder-prefixed bullets.
6. For writes: restate the proposed change in one line and wait for
   an explicit confirmation (👍, "yes", "go", "do it") in the same
   thread before executing. Reply with the row reference (funder +
   due date) and a link to the spreadsheet only after executing.
